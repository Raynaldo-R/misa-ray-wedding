/* Guest mini-game leaderboards — client-side localStorage */
(function (global) {
  'use strict';
  ...
  function sanitizeName(raw) {
    var s = stripHtml(String(raw || '')).trim();
  ...
  function getTopScores(categoryId, limit) {
    var cat = getCategory(categoryId);
    if (!cat) return [];
    return sortEntries(cat, loadEntries(cat)).slice(0, limit || TOP_N);
  }
  ...
  function render(container, categoryId, opts) {
    ...
    var entries = getTopScores(categoryId, opts.limit);
    if (!entries.length) {
  ...
  function buildPromptForm(opts) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (typeof opts.onSubmit === 'function') opts.onSubmit(name);
    });

    return { wrap: wrap, input: input, form: form };
  }
  ...
  function promptScoreSubmit(opts) {
    ...
    return promptName({
      ...
      onSubmit: function (name) {
        var ok = submitScore(opts.category, name, opts.score);
